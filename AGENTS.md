...
- `.agents/`: Canonical Markdown agent sources (single source of truth) —
  shared frontmatter (`name`, `description`, `color`) plus the full body. No
...
- `.config/opencode/agents/`: OpenCode agent definitions (generated Markdown,
  git-ignored).
...
1. **Persona Consistency**: Agents are authored once in `.agents/` (YAML
   ...
   to materialize the per-platform copies. Claude and OpenCode copies remain
   ...
   because it is not a supported custom-agent field. Every agent's effective
   per-platform `model` (and `temperature` only where supported, currently
   OpenCode) is listed explicitly in `platforms.<name>.agents.<slug>`, with a
   `description` present only where it diverges from the canonical source;
   platform-exclusive agents declare a `platforms:` list in their canonical
   frontmatter (only agents that exist on a subset of platforms — most agents
   are shared and declare no `platforms:` line, meaning all platforms).
