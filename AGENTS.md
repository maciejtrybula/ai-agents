...
### Project Structure
- `.agents/`: Canonical agent sources (single source of truth) — shared
  frontmatter (`name`, `description`, `color`) plus the full body. No
  `model`/`temperature` here.
- `.claude/agents/`: Claude agent definitions (generated by
...
1. **Persona Consistency**: Agents are authored once in `.agents/` (YAML
   frontmatter `name`, `description`, `color`). Run `./generate-agents.sh`
   to materialize the per-platform copies; per-platform `model` (and
   whether `color` is emitted) come from `.config/agent-platforms.json`.
   Every agent's effective per-platform `model` (and, where the platform
   has that concept, `temperature`) is listed explicitly in
   `platforms.<name>.agents.<slug>`, with a `description` present only
   where it diverges from the canonical source; platform-exclusive
   agents declare a `platforms:` list in their canonical frontmatter
   (only agents that exist on a subset of platforms — most agents are
   shared and declare no `platforms:` line, meaning all platforms).
